...
func HandleDockerEvents() {
	...
	for _, c := range containers {
		...
		cInspection, err := mustInspectContainer(ctx, c.ID)
		if err != nil {
			logging.Errorf("status=inspect-container-first, solver=docker, err=%v", ctx, err)
		}
	...
	for {
		...
		if err != nil && err == io.EOF {
			logging.Errorf("status=decoding-error, err=%s", err)
			break
		}

		cInspection, err := mustInspectContainer(ctx, event.ID)
		if err != nil {
			logging.Errorf("status=inspect-container-on-event, id=%s, err=%s", event.ID, err)
			continue
		}
		hostnames := getHostnames(cInspection)
		...
		case "start":
			...
			cInspection, err := mustInspectContainer(ctx, event.ID)
			if err != nil {
				logging.Errorf("status=inspect-container-on-start, id=%s, err=%s", event.ID, err)
				continue
			}
			putHostnames(ctx, hostnames, cInspection)
			break
...
func mustInspectContainer(ctx context.Context, containerID string) (types.ContainerJSON, error) {
	cInspection, err := dockernetwork.GetCli().ContainerInspect(ctx, containerID)
	if err != nil {
		return types.ContainerJSON{}, errors.Wrap(err, "container inspect")
	}

	return cInspection, nil
}
